function fetchBarangaySummary(municipalityId) {
    $.ajax({
        ...
        success: function (response) {
            let summaryHtml = "<strong>Barangay Totals:</strong><br> ";
            if (Object.keys(response).length === 0) {
                summaryHtml += "No data available.";
            } else {
                summaryHtml += Object.entries(response)
                    .map(([brgy, count]) => {
        ...
        error: function () {
            $("#barangaySummary").html(
                '<span class="text-danger">Error fetching barangay totals.</span>'
            );
        },
    });
}
