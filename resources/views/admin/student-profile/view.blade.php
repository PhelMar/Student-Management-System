...
<div class="card">
    ...
    <div class="card-body">
        ...
        <a href="javascript:void(0)" onclick="confirmGraduate('{{ $students->id }}')" class="btn btn-warning btn-md">Graduate</a>
        <a href="javascript:void(0)" onclick="confirmDelete('{{ $students->id }}')" class="btn btn-danger btn-md">Delete</a>
    </div>
</div>
<script>
    function confirmGraduate(studentId) {
        Swal.fire({
            title: 'Are you sure?',
            ...
            if (result.isConfirmed) {
                window.location.href = "{{ url('admin/students/graduate') }}/" + studentId;
            }
        });
    }
    ...
    function confirmDelete(studentId) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                fetch("{{ url('admin/students/delete') }}/" + studentId, {
                        method: 'DELETE',
                        headers: {
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            window.location.href = "{{ route('admin.students.display') }}";
                        } else {
