...
<div class="card shadow-xl">
    ...
    <div class="card-body overflow-auto" style="max-height: 500px;">
        <form action="{{ route('admin.students.storeStudent') }}" method="post" id="addStudentForm">
            @csrf
...
<script>
    const checkEmailUrl = "{{ route('admin.students.checkEmailStudent') }}";
    const csrfToken = '{{ csrf_token() }}';

    const checkIDNoUrl = "{{ route('admin.students.checkIDNoStudent') }}";
    const muncipalitiesUrl = "{{ route('admin.municipalitiesStudent', ':province_id') }}";
    const barangaysUrl = "{{ route('admin.barangaysStudent', ':municipality_id') }}";
</script>
