...
<div class="row justify-content-end align-items-center mb-3">
    ...
    <div class="col-sm-auto">
        <a id="printButton" href="{{ route('user.solo-parent-student.print') }}" target="_blank" class="btn btn-primary">Print</a>
...
<script>
    ...
    document.getElementById('pdfDownload').addEventListener('click', function() {
        ...
        if (schoolYearId) {
            window.location.href = "{{ route('user.export.soloparentpdf') }}?school_year_id=" + schoolYearId;
        } else {
    ...
    document.getElementById('printButton').addEventListener('click', function(event) {
        ...
        var schoolYearId = document.getElementById('school_year_id').value;
        if (schoolYearId) {
            window.open("{{ route('user.solo-parent-student.print') }}?school_year_id=" + schoolYearId);
        } else {
            alert("Please select a school year.");
        }
    });
